...
export default async function buscarProcesso(params: any): Promise<void> {
  try {
    if (Array.isArray(params) && params.length === 1 && typeof params[0] === 'object') {
      params = params[0];
    }
    const { tribunal, numeroProcesso } = params;
    if (!tribunal || !numeroProcesso) {
      return console.log(docgo.result(false, null, 'Parâmetros obrigatórios: tribunal, numeroProcesso'));
    }
